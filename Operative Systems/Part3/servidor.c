 ...
 **
 ** Aluno: Nº: 99377       Nome: Leonardo Manuel Dos Reis Pinto De Andrade
 ** Nome do Módulo: servidor.c v2 (https://moodle22.iscte-iul.pt/mod/forum/discuss.php?d=6117#p10339)
 ** Descrição/Explicação do Módulo:
 **     Este módulo funciona de acordo com o enunciado, havendo comentários presentes em todo o lado para facilitar a leitura do código

 **     Notas acerca de erros após correr o validador até a altura que isto foi escrito: O validador não devolve corretamente o valor
 **     atual dos semáforos pois a função semctl( semId, semNr, GETVAL, 0 ) devolve sempre 0. Isto afeta todas as outras funções
 **     auxiliares criadas pelos professores que dizem respeito à obtenção ou alteração do valor dos semáforos.
 **     As mensagens de debug provam isto tudo. 
 ******************************************************************************/
#include "common.h"
...
int initShm_S1() {
    ...
    if(result >= 0){
        db = shmAttach(result);
        if(db != NULL)
            so_success("S1.1", "%d", result); 
    } else {                    // Se deu erro ao tentar abrir uma SHM com a Key IPC_KEY
        so_error("S1.1", "");

        if(errno != ENOENT)         // Falhou por outros motivos
            so_error("S1.2", "");
        else {                          // Não havia SHM criada
            so_success("S1.2", "");
            
            result = shmCreate(1);
            if(result < 0)          // Falhou ao criar
                so_error("S1.3", "");
            else {                      // Criou com sucesso
                so_success("S1.3", "%d", result); 

                // Ligar a db à SHM
                db = shmAttach(result);
                for(int i = 0; i < MAX_USERS; i++){         // Iniciar a lista de Utilizadores
                    db->listUsers[i].nif = USER_NOT_FOUND;
                }
                for(int i = 0; i < MAX_PRODUCTS; i++){      // Iniciar a lista de Produtos
                    db->listProducts[i].idProduto = PRODUCT_NOT_FOUND;
                }
                so_success("S1.4", "");

                // S1.5
                FILE *bd_utilizadores = fopen(FILE_DATABASE_USERS, "r");    // Abrir o ficheiro para leitura
                if(!bd_utilizadores){
                    so_error("S1.5", "");
                    result = RET_ERROR;
                } else {
                    int index_aux = 0;
                    size_t user_read;
                    while((user_read = fread(&db->listUsers[index_aux], sizeof(Login), 1, bd_utilizadores)) == 1){
                        db->listUsers[index_aux].pidCliente = -1;
                        db->listUsers[index_aux].pidServidorDedicado = -1;
                        index_aux++;
                    }

                    if(index_aux == 0){     // Se ocorreu um erro na leitura ou se o ficheiro estava vazio
                        so_error("S1.5", "");
                        result = RET_ERROR;
                    } else {                // Leu e preencheu a lista de Utilizadores corretamente
                        fclose(bd_utilizadores);
                        so_success("S1.5", "");
                    }
                }

                // S1.6
                FILE *bd_produtos = fopen(FILE_DATABASE_PRODUCTS, "r");     // Abrir o ficheiro para leitura
                if(!bd_produtos){
                    so_error("S1.6", "");
                    result = RET_ERROR;
                } else {
                    int index_aux1 = 0;
                    size_t product_read;
                    while((product_read = fread(&db->listProducts[index_aux1], sizeof(Produto), 1, bd_produtos)) == 1){
                        index_aux1++;
                    }

                    if (index_aux1 == 0) {      // Se ocorreu um erro na leitura ou se o ficheiro estava vazio
                        so_error("S1.6", "");
                        result = RET_ERROR;
                    } else {
                        fclose(bd_produtos);
...
int initMsg_S2() {
    ...
    result = msgGet(); 
    if(result >= 0){    // Se devolver um nº não negativo então é pq já existe
        if(msgRemove(result) < 0){
            so_error("S2.1", "");
            result = RET_ERROR;
        } else{
            so_success("S2.1", ""); // Se foi preciso remover e fez com sucesso
            ...
            if(result < 0)
                ...
                so_success("S2.2", "%d", result);
        }
    } 
    else{
        so_success("S2.1", "");     // Se não foi preciso remover
...
int initSem_S3() {
    ...
    result = semGet(); 
    if(result >= 0){    // Se devolver um nº não negativo então é pq já existe SEM e vamos remover
        if(semRemove(result) < 0){
            so_error("S3.1", "");
            result = RET_ERROR;
        } else{
            so_success("S3.1", ""); // Se foi preciso remover e fez com sucesso

            // Criar o SEM
            result = semCreate(3); 
            if(result < 0)  // Erro na criação do SEM
                so_error("S3.2", "");
            else{   // SEM criado com sucesso
                so_success("S3.2", "%d", result);
                if(semNrSetValue(result, SEM_USERS, SEM_MUTEX_INITIAL_VALUE) < 0 || // Erros durante a incialização dos semáforos
                    semNrSetValue(result, SEM_PRODUCTS, SEM_MUTEX_INITIAL_VALUE) < 0 || semNrSetValue(result, SEM_NR_SRV_DEDICADOS, 0) < 0){
                    so_error("S3.3", "");
                    result = RET_ERROR; 
                } else  // Inicialização dos semáforos com sucesso
                    so_success("S3.3", "%d", result);
            }
        }
    } else {          // Se SEM não existir
        so_success("S3.1", ""); // Não foi preciso remover o SEM pois não existia
        ...
        if(result < 0)
            ...
            so_success("S3.2", "%d", result);
            if(semNrSetValue(result, SEM_USERS, SEM_MUTEX_INITIAL_VALUE) < 0 || 
                semNrSetValue(result, SEM_PRODUCTS, SEM_MUTEX_INITIAL_VALUE) < 0 || semNrSetValue(result, SEM_NR_SRV_DEDICADOS, 0) < 0){
                so_error("S3.3", "");
                result = RET_ERROR; 
            } else
...
int createServidorDedicado_S6() {
    ...
    pid_filho = fork();
    if(pid_filho == -1)     // Erro com o fork
        so_error("S6", "");
    else if(pid_filho != 0)     // Processo Servidor (pai)
...
void shutdownAndExit_S7() {
    ...
    if(shmId >= 0 && db != NULL){  // Existe SHM aberta e alocada (Apenas seria necessário verificar o db mas o validador não funciona o permite)
        so_success("S7.1", "");
        ...
        // S7.4
        FILE *bd_produtos = fopen(FILE_DATABASE_PRODUCTS, "w");
        if(!bd_produtos)    // Erro na abertura do ficheiro
            so_error("S7.4", "");
        else {      // O ficheiro foi aberto com sucesso
            int error = 0;      // Variável de controlo no caso de qualquer erro durante a escrita do ficheiro
            for(int i1 = 0; i1 < MAX_PRODUCTS; i1++){
                if(db->listProducts[i1].stock > 0){
                    // Escrever no ficheiro
                    size_t elements_written = fwrite(&db->listProducts[i1], sizeof(Produto), 1, bd_produtos);
                    if(elements_written != 1)
                        error = -1;
                }
            }

            if(error == -1) // Se houver erros durante a escrita
                so_error("S7.4", "");
            else {      // Se escreveu sem problemas
                so_success("S7.4", "");
                fclose(bd_produtos);
            }
        }
    } else {
        so_error("S7.1", "");   // Não existe SHM aberta e/ou alocada
    }

    // S7.5
    if(semRemove(semId) < 0 || shmRemove() < 0 || msgRemove(msgId) < 0)
        so_error("S7.5", "");
    else
        so_success("S7.5", "");
...
void handleSignalSIGCHLD_S9(int sinalRecebido) {
    ...
    pid_t sd_pid = wait(NULL);    // wait vai retornar o pid do Servidor Dedicado que terminou

    // Se sd_pid for válido
    if(sd_pid != -1)
        so_success("S9", "Terminou Servidor Dedicado %d", sd_pid);
...
int searchUserDB_SD12(Login request) {
    ...
    for(int i = 0; i < MAX_USERS; i++){
        if(db->listUsers[i].nif == request.nif && strcmp(db->listUsers[i].senha, request.senha) == 0){  // Se encontrou dados que façam match na db
            indexClient = i;    // Retornar o indexClient
            so_success("SD12.1", "%d", indexClient);
        }
    }
    
    if(indexClient == USER_NOT_FOUND){  // Se não encontrou o user na db enviamos uma mensagem de erro e retornamos USER_NOT_FOUND
        so_error("SD12.1", "");
        msg.msgData.infoLogin.pidServidorDedicado = -1;     // Visto que request não é um pointer não posso fazer request.pidServidorDedicado = -1;
        if(msgSend(msgId, msg, request.pidCliente) < 0) // Erro no envio da mensagem ao Cliente
            so_error("SD12.2", "");
        else    //
            so_success("SD12.2", "");
    }
...
int reserveUserDB_SD13(int indexClient, int pidCliente) {
    ...
    semNrDown(semId, SEM_USERS);    // Bloquear a SHM de outros processos
    db->listUsers[indexClient].pidServidorDedicado = getpid();
    db->listUsers[indexClient].pidCliente = pidCliente;
    so_success("SD13", "");
    result = RET_SUCCESS;
    semNrUp(semId, SEM_USERS);  // Libertar a SHM para os outros processos poderem aceder

    /*so_debug("> [@pidServidorDedicado:%d @pidCliente:%d]", db->listUsers[indexClient].pidServidorDedicado, 
                                                            db->listUsers[indexClient].pidCliente); */
    so_debug("> [@return:%d]", result);
    return result;
}
...
int sendProductList_SD14(int indexClient, int pidCliente) {
    ...
    // Preencher informação sobre o Login da mensagem
    ...
    // Enviar a lista de produtos disponiveis
    for(int i = 0; i < MAX_PRODUCTS; i++){
        if(db->listProducts[i].stock > 0){
            memcpy(&msg.msgData.infoProduto, &db->listProducts[i], sizeof(Produto));
            if(msgSend(msgId, msg, pidCliente) < 0){
                so_error("SD14.2", "");
                break;
            }
        }
        if(i == MAX_PRODUCTS-1) // final da lista
            so_success("SD14.2", "");
    }

    // Enviar msg para o cliente que avisa não haver mais produtos a listar
...
MsgContent receiveClientOrder_SD15() {
    ...
    if(msgReceive(msgId, &msg, getpid()) < 0)   // Erro na leitura da resposta do Cliente
        so_error("SD15", "");
    else    // Resposta lida com sucesso
        so_success("SD15", "");
...
int sendPurchaseAck_SD16(int idProduto, int pidCliente) {
    ...
    if(idProduto == PRODUCT_NOT_FOUND){     // Se o ID introduzido pelo cliente não existe 
        msg.msgData.infoProduto.idProduto = PRODUTO_NAO_COMPRADO;
        so_error("SD16.1", "");
    } else {
        semNrDown(semId, SEM_PRODUCTS);     // Bloquear a SHM de outros processos

        int productFound;     
        for(int i = 0; i < MAX_PRODUCTS; i++){  // Percorer a lista de produtos
            if(db->listProducts[i].idProduto == idProduto && db->listProducts[i].stock > 0){    // Se encontrou e tem stock disponível
        ...
        if(productFound != 1){   // Chegou ao final da lista e não encontrou
            msg.msgData.infoProduto.idProduto = PRODUTO_NAO_COMPRADO;
            so_error("SD16.2", "");      
        }

        semNrUp(semId, SEM_PRODUCTS);   // Libertar a SHM para os outros processos poderem aceder
    }
    
    // Enviar mensagem/resposta final para o Cliente
    if(msgSend(msgId, msg, pidCliente) < 0)
        so_error("SD16.3", "");
    else{
        so_success("SD16.3", "");
