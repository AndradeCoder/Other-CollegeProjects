 ...
 **     Este módulo funciona de acordo com o enunciado, havendo comentários presentes em todo o lado para facilitar a sua leitura.
 **     Acrescenta-se que em todas as funções que resultam na terminação do Cliente, em casos erro, foi implementado a remoção da MSG
 ******************************************************************************/
#include "common.h"
...
int initMsg_C1() {
    ...
    result = msgGet();      // msgID
    if(result < 0){     // Erro ao abrir
        so_error("C1", "");
        msgRemove(result);
    } else                  // MSG foi aberta com sucesso
...
int triggerSignals_C2() {
    ...
    if(signal(SIGUSR2, trataSinalSIGUSR2_C10) == SIG_ERR || signal(SIGINT, trataSinalSIGINT_C11) == SIG_ERR || 
                                                    signal(SIGALRM, trataSinalSIGALRM_C12) == SIG_ERR){
        so_error("C2", "");
        msgRemove(msgId);
    } else {
        so_success("C2", "");
...
MsgContent getDadosPedidoUtilizador_C3() {
    ...
    int nif = so_geti();    // Obter nif
    char *senha = so_gets(buffer, MAX_BUFFER_SIZE); // Obter senha
    
    if(!nif || strlen(senha) <= 0){     // Em caso de erro na escrita de algum deles
        so_error("C3", "");
        msgRemove(msgId);
    } else {
        msg.msgData.infoLogin.nif = nif;
        strcpy(msg.msgData.infoLogin.senha, senha); // única forma de copiar strings
        msg.msgData.infoLogin.pidCliente = getpid(); // PID do seu próprio processo Cliente
...
int sendClientLogin_C4(MsgContent msg) {
    ...
    if(msgSend(msgId, msg, MSGTYPE_LOGIN) < 0)  // Se houve erros ao enviar a mensagem
        so_error("C4", "");
    else{      // Caso contrário
        so_success("C4", "");
        result = RET_SUCCESS;
...
int receiveProductList_C6() {
    ...
    if(msgReceive(msgId, &msg, getpid()) < 0){  // Erro ao ler a mensagem do Servidor
        so_error("C6", "");
        msgRemove(msgId);
    } else {    // Leu a mensagem com sucesso
        ...
        if(msg.msgData.infoLogin.pidServidorDedicado == -1){    // Autenticação sem sucesso
            so_error("C6.2", "");
            msgRemove(msgId);
        } else {                // Autenticação com sucesso
            printf("Lista de Produtos Disponíveis:\n");
            while(1){   // Ciclo responsável pela listagem dos produtos
                if(msg.msgData.infoProduto.idProduto == FIM_LISTA_PRODUTOS){    // Não há mais produtos a listar, terminar o ciclo e função
                    so_success("C6.3", "");
                    result = RET_SUCCESS;
                    break;
                }
                printf("idProduto:%d Nome:%s Categoria:%s Preço:%d\n", msg.msgData.infoProduto.idProduto, // Informações do produto
                            msg.msgData.infoProduto.nomeProduto, msg.msgData.infoProduto.categoria, msg.msgData.infoProduto.preco);

                if(msgReceive(msgId, &msg, getpid()) < 0){  // Ler o próximo produto da lista. Em caso de erro termina o Cliente
                    so_error("C6.5", "");
                    msgRemove(msgId);
                    break;
                } else continue;
...
int getIdProdutoUtilizador_C7() {
    ...
    result = so_geti();     // idProduto inserido pelo utilizador
    if(!result){        // Em caso de erro ao escrever
        so_error("C7", "");
        result = PRODUCT_NOT_FOUND;
    } else                  // Escrita do idProduto com sucesso
...
int sendClientOrder_C8(int idProduto, int pidServidorDedicado) {
    ...
    if(msgSend(msgId, msg, pidServidorDedicado) < 0)    // Em caso de erro ao enviar a mensagem para o Servidor
        so_error("C8", "");
    else{                       // Envio da mensagem com sucesso
        so_success("C8", "");
        result = RET_SUCCESS;
    }
...
void receivePurchaseAck_C9() {
    ...
    if(msgReceive(msgId, &msg, getpid()) < 0)   // Erro ao ler a resposta final do Servidor
        so_error("C9", "");
    else{                              // Leitura da mensagem com sucesso
