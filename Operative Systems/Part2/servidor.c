 ...
 ** Descrição/Explicação do Módulo: O módulo funciona como é pretendido. Náo utilizo o passo SD17
 ** na minha função main, portanto, sempre que um passo passa para esse passo é através de uma invocação do método.
 ** Apenas existe 1 erro que é a atribuição correta do valor pid_servidor_dedicado no passo SD12.
 **
 **
 ******************************************************************************/
#include "common.h"
...
int triggerSignals_S3() {
    ...
    //sighandler_t s7 = signal(SIGINT, trataSinalSIGINT_S7);
    //sighandler_t s8 = signal(SIGCHLD, trataSinalSIGCHLD_S8);
...
Login readRequest_S4(char *nameFifo) {
    ...
    size_t info_read = fread(&request, sizeof(Login), 1, fifo);
    if(info_read == 0){
        so_error("S4", "");
        return request;
    }
...
void trataSinalSIGINT_S7(int sinalRecebido) {
    ...
    if(sinalRecebido == SIGINT){
        ...
        FILE *bd_utilizadores = fopen(FILE_DATABASE, "r");
        if(!bd_utilizadores){
            so_error("S7.1", "");
            deleteFifoAndExit_S6();
        }
            ...
            so_success("S7.1", "");
            size_t info_read = fread(&clientRequest, sizeof(Login), 1, bd_utilizadores);
            if(info_read == 0){
                so_error("S7.2", "");
                deleteFifoAndExit_S6();
            }
            else{
                while(fread(&clientRequest, sizeof(Login), 1, bd_utilizadores) == 1){
...
void trataSinalSIGCHLD_S8(int sinalRecebido) {
    ...
    //int status;
    pid_t sd_pid = wait(&sinalRecebido);

    // se sd_pid for maior que 0 significa que um servidor dedicado terminou
    if(sd_pid > 0)
        so_success("S8", "Terminou Servidor Dedicado %d", sd_pid);
...
int triggerSignals_SD9() {
    ...
    //sighandler_t sd18 = signal(SIGUSR1, trataSinalSIGUSR1_SD18);
    //sighandler_t ignore = signal(SIGINT, SIG_IGN);
...
int procuraUtilizadorBD_SD11(Login request, char *nameDB, Login *itemDB) {
    ...
    FILE *bd_utilizadores = fopen(nameDB, "r");
    if(!bd_utilizadores){
        so_error("SD11", "");
    }
    so_success("SD11", "");
    ...
    Login client_InFile;
    while (fread(&client_InFile, sizeof(Login), 1, bd_utilizadores) == 1) {
        index_client++;
        if (client_InFile.nif == request.nif) {
            if (strcmp(client_InFile.senha, request.senha) == 0) {
                *itemDB = client_InFile;
                fclose(bd_utilizadores);
...
int reservaUtilizadorBD_SD12(Login *request, char *nameDB, int index_client, Login itemDB) {
    ...
    if(index_client < 0){
        ...
        strncpy(request->nome, itemDB.nome, strlen(itemDB.nome));
        request->saldo = itemDB.saldo;
        request->pid_servidor_dedicado = getpid();

        FILE *bd_utilizadores = fopen(nameDB, "rb+");
        if (!bd_utilizadores){
            so_error("SD12.2", "Erro a abrir ficheiro de utilizadores");
            return -1;
        }
        so_success("SD12.2", "Ficheiro de utilizadores aberto com sucesso");

        int seek = fseek(bd_utilizadores, index_client*sizeof(Login), SEEK_SET); 
        if(seek < 0){
            so_error("SD12.3", "Erro ao posicionar apontador do ficheiro");
            fclose(bd_utilizadores);
            return -1;
        }
        so_success("SD12.3", "Apontador do ficheiro posicionado com sucesso");

        fseek(bd_utilizadores, -1*sizeof(Login), SEEK_CUR);
        int pid_cliente = itemDB.pid_cliente;
        int pid_servidor_dedicado = getpid();
        if(fwrite(&pid_cliente, sizeof(int), 1, bd_utilizadores) != 1 || 
                                                    fwrite(&pid_servidor_dedicado, sizeof(int), 1, bd_utilizadores) != 1){
            so_error("SD12.4", "Erro ao escrever no ficheiro");
            fclose(bd_utilizadores);
            return -1;
        }
        so_success("SD12.4", "Informação atualizada com sucesso no ficheiro de utilizadores");
        fclose(bd_utilizadores);
...
int buildNomeFifo(char *buffer, int buffer_size, char *prefix, int pid, char *suffix) {
    ...
    int len = snprintf(buffer, buffer_size, "%s%d%s", prefix, pid, suffix);
    if(len >= 0 && len < buffer_size){ /* Meteu-se >=0 porque apesar de não ser o caso do trabalho, buffer_size pode ser > 0. 
                                        É só de maneira a ter o código o mais otimizado possível */
        result = 0;
    }
...
int sendAckLogin_SD14(Login ackLogin, char *nameFifo) {
    ...
    if(!fifo){
        ...
        size_t elements_written = fwrite(&ackLogin, sizeof(Login), 1, fifo);
...
int readFimSessao_SD15(char *nameFifo) {
    ...
    int MAX_BUFFER_SIZE = sizeof(Login);
    char buffer[MAX_BUFFER_SIZE];
    char *typed = so_gets(buffer, MAX_BUFFER_SIZE);
    if(strlen(typed) <= 0){
        so_error("SD15", "");
        deleteFifoAndExit_SD17();
        //result = 0;
...
int terminaSrvDedicado_SD16(Login *clientRequest, char *nameDB, int index_client) {
    ...
    if(bd_utilizadores){
        ...
        int seek = fseek(bd_utilizadores, index_client*sizeof(Login), SEEK_SET);
        if(seek < 0){
            so_error("SD16.3", "HAHAHAH");
            deleteFifoAndExit_SD17();
        }
        else{
            so_success("SD16.3", "");
            fseek(bd_utilizadores, -1*sizeof(Login), SEEK_CUR);
            size_t elements_written = fwrite(clientRequest, sizeof(clientRequest), 1, bd_utilizadores);
            if(elements_written != 1){
                so_error("SD16.4", "");
                deleteFifoAndExit_SD17();
            }
...
void deleteFifoAndExit_SD17() {
    ...
    int i = unlink(nameFifoServidorDedicado);
    if(i == 0)
        so_success("SD17", "");
    else
        so_error("SD17", "");
