 ...
 ** Descrição/Explicação do Módulo: Este módulo funciona como é pretendido. Apenas utilizo o passo C11 no meu main
 ** para o propósito da passagem do C10 para C11 sem haver erros (de terminação?) no validador. 
 **
 **
 ******************************************************************************/
#include "common.h"
...
int triggerSignals_C2() {
    ...
    if(signal(SIGUSR2, trataSinalSIGUSR2_C12) == SIG_ERR || signal(SIGINT, trataSinalSIGINT_C13) == SIG_ERR || 
                                                    signal(SIGALRM, trataSinalSIGALRM_C14) == SIG_ERR)
        so_error("C2", "");
    else{
        so_success("C2", "");
        result = 0;
...
Login getDadosPedidoUtilizador_C3() {
    ...
    char *senha = so_gets(buffer, MAX_BUFFER_SIZE);
    
    if(!nif || strlen(senha) <= 0)
        so_error("C3", "");
    else{
        request.nif = nif;
        strcpy(request.senha, senha); // Como não posso fazer request.senha = senha, faço strcpy do conteudo de senha -> request.senha
        request.pid_cliente = getpid(); // PID do seu próprio processo Cliente
...
int writeRequest_C5(Login request, char *nameFifo) {
    ...
    if(!fifo){
        ...
        size_t elements_written = fwrite(&request, sizeof(Login), 1, fifo);
...
Login readAckLogin_C7(char *nameFifo) {
    ...
    if(!fifo){
        so_error("C7", "Error opening fifo %s: %s", nameFifo, strerror(errno));
        deleteFifoAndExit_C11();
    }
    else{
        size_t info_read = fread(&ackLogin, sizeof(Login), 1, fifo);
        if(info_read < 1){
            if(ferror(fifo)){
                so_error("C7", "Error reading from fifo %s: %s", nameFifo, strerror(errno));
            } else {
                so_error("C7", "Fifo %s is empty or has been closed by the other end", nameFifo);
            }
            deleteFifoAndExit_C11();
        }
...
int writeFimSessao_C10(char *nameFifo, int tempoDeProcessamento, Login ackLogin) {
    ...
    if(!fifo)
        ...
        int typed = fprintf(fifo, "Sessão Login ativa durante %d segundos\n", tempoDeProcessamento);
        if(typed <= 0)
            so_error("C10", "");
        else{
            so_success("C10", "");
...
void trataSinalSIGINT_C13(int sinalRecebido) {
    ...
    if(sinalRecebido == SIGINT){
        ...
        if(!fifo)
            ...
            int typed = fprintf(fifo, "Sessão cancelada pelo utilizador\n");
            if(typed <= 0)
                so_error("C13", "");
            else
                so_success("C13", "");
